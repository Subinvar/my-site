# My Site

Интема Групп — двуязычный корпоративный сайт на Next.js 16 с интеграцией [Keystatic](https://keystatic.com) для редактирования контента напрямую из GitHub. Репозиторий уже настроен для русской (по умолчанию) и английской локалей, генерации Atom-лент, sitemap, а также для предпросмотра и публикации статей блога.

## Обзор стека

- **Next.js 16 (App Router)** — серверный рендеринг, статическая генерация и динамические маршруты для локалей.
- **TypeScript** — строгая типизация UI и серверного кода.
- **Keystatic** — headless CMS с GitHub-потоком и UI по адресу `/keystatic`.
- **Tailwind/PostCSS** — стилизация компонентов и глобальных стилей.
- **pnpm** — менеджер пакетов и монорепозиторный workspace.

## Основные страницы и адреса

- `/` и `/en` — главная страница с переключателем языка.
- `/o-kompanii` и `/en/about` — раздел «О компании».
- `/contacts` и `/en/contacts` — страница с формой обратной связи (отправляет письма через SMTP, см. переменные окружения ниже).
- `/catalog` и `/en/catalog` — список продукции с фильтрами; карточки товаров строятся по данным из `content/catalog/*`.
- `/products` и `/en/products` — обзорный раздел с подборками категорий каталога (связующие, покрытия, вспомогательные материалы).
- `/partners` и `/en/partners` — страница с информацией для партнёров и дилеров.
- `/documents` и `/en/documents` — библиотека документов и сертификатов.
- `/news` и `/en/news` — раздел с новостями компании и практическими материалами, список собирается из коллекции `pages/news` и связанных записей.
- `/news/[slug]` и `/en/news/[slug]` — страницы новостей/кейсов, формируются динамически по данным Keystatic.
- `/keystatic` — интерфейс админки для редактирования контента.

## Быстрый старт

```bash
pnpm install
pnpm dev
```

После запуска приложение доступно на [http://localhost:3000](http://localhost:3000). Интерфейс Keystatic (редактор контента) открыт по адресу [http://localhost:3000/keystatic](http://localhost:3000/keystatic).

## Переменные окружения

Для сборки и работы Keystatic в GitHub-режиме нужно создать файл `.env.local` (локально) и добавить переменные в настройках продакшн-окружения (например, Vercel). Секреты читаются только на серверной стороне (API `/api/keystatic` и middleware) напрямую из `process.env`, их не нужно прокидывать через `next.config.ts` или доступные клиенту `NEXT_PUBLIC_*` переменные. В качестве основы можно использовать `.env.example`:

```ini
KEYSTATIC_GITHUB_CLIENT_ID=...
KEYSTATIC_GITHUB_CLIENT_SECRET=...
KEYSTATIC_SECRET=...
NEXT_PUBLIC_KEYSTATIC_GITHUB_APP_SLUG=...
# Опционально: включите Basic Auth только для админки Keystatic
# чтобы браузер показывал диалог логина вместо пустого экрана, если кто-то блокирует авторизацию
KEYSTATIC_BASIC_AUTH_USER=admin
KEYSTATIC_BASIC_AUTH_PASS=super-secret
```

- `KEYSTATIC_GITHUB_CLIENT_ID` и `KEYSTATIC_GITHUB_CLIENT_SECRET` — данные OAuth-приложения GitHub.
- `KEYSTATIC_SECRET` — секрет для подписи токенов Keystatic.
- `NEXT_PUBLIC_KEYSTATIC_GITHUB_APP_SLUG` — slug установленного GitHub App.
- `KEYSTATIC_BASIC_AUTH_USER` и `KEYSTATIC_BASIC_AUTH_PASS` — включают Basic Auth только для `/keystatic` и `/api/keystatic`. Если не заданы, админка открывается без логина/пароля. Диалог авторизации предотвращает ситуацию с «пустым экраном», когда браузер блокирует запросы без учётных данных.

Keystatic всегда работает в GitHub Mode и коммитит прямо в ветку `main` этого репозитория. Поэтому достаточно добавить эти четыре переменные в `.env.local` (для локальной разработки) и в настройки окружений Vercel (preview/production). После деплоя админка `/keystatic` сразу потребует авторизацию через GitHub и будет писать изменения непосредственно в репозиторий.

**Как проверить GitHub Mode:**

1. Заполните `.env.local` переменными из списка выше и запустите `pnpm dev`.
2. Откройте [http://localhost:3000/keystatic](http://localhost:3000/keystatic) — интерфейс должен перенаправить в GitHub OAuth и после логина показать контент репозитория.
3. Выполните `pnpm build`, чтобы убедиться, что production-сборка использует тот же GitHub storage без дополнительных переменных.

> ⚠️ Не коммитьте реальные значения секретов в репозиторий.

При необходимости для работы с системными сертификатами Node.js можно включить:

```ini
NEXT_TURBOPACK_EXPERIMENTAL_USE_SYSTEM_TLS_CERTS=1
```

Для работы серверной формы контактов добавьте SMTP-настройки (локально в `.env.local` и в переменные Vercel):

```ini
SMTP_HOST=... # хост SMTP-сервера (например, smtp.yandex.ru)
SMTP_PORT=587 # порт: 465 для SSL или 587 для STARTTLS
SMTP_USER=... # логин/отправитель
SMTP_PASS=... # пароль или App Password
LEADS_TO=...  # адрес получателя лидов
```

Значения берите у вашего почтового/SMTP‑провайдера или сервисов вроде Postmark, Mailgun, Yandex/Google (с app password).

## Структура контента

- `content/site/index.json` — глобальные настройки (название, контакты, SEO, домен).
- `content/home/index.json` — данные главной страницы (Hero, направления, карусели).
- `content/pages/<slug>/index.json` — данные страниц «Главная» и «О компании» на двух языках. Сам контент хранится в файлах `content/pages/<slug>/content/{locale}.mdoc`.
- `content/catalog/<slug>/index.json` — карточки продуктов каталога с локализованными слагами, характеристиками и Markdoc-контентом в `content/catalog/<slug>/content/{locale}.mdoc`.
- `content/posts/<slug>/index.json` — метаданные записей блога, а тексты лежат в `content/posts/<slug>/content/{locale}.mdoc`.
- `content/documents/<slug>/index.json` — карточки документов и сертификатов с типом, языком, файлом и связью с продуктами каталога.
- `content/documents-page/index.json` — тексты и подписи для страницы «Документы и сертификаты», включая фильтры.
- `content/catalog-page/index.json` — тексты для списка каталога (фильтры, заголовки, заглушки).
- `content/navigation/index.json` — ссылки шапки и подвала сайта.
- `content/dictionary/index.json` — словарь интерфейса (тексты ошибок, aria-лейблы, подписи таблиц ленты, подсказки для каталога, заголовок страницы карты сайта).
- `content/catalog-*` — справочники для фильтров каталога (процессы, основы, наполнители, металлы, вспомогательные материалы) и набор категорий для страницы `/products`.
- `content/pages/Новости` — контент и описание страницы `/news` с текстом о разделе и автоматическим списком записей.
- `public/uploads` — изображения для страниц и social preview.

Все коллекции синхронизируются через Keystatic: при редактировании в UI изменения пишутся в Git и доступны через Pull Request.

## Файл SITEMAP

В корне репозитория лежит текстовый файл `SITEMAP`. Это вспомогательная «живая» карта сайта, которую удобно обновлять вручную, когд
а появляются новые продукты, документы или посты. Она дополняет автоматически генерируемый `sitemap.xml` и помогает быстро проверить
, что у контент-страниц есть оба варианта локализации.

Рекомендуемый порядок при добавлении контента через Keystatic:

1. Создать или отредактировать запись в нужной коллекции.
2. Убедиться, что новая страница открывается в обеих локалях.
3. Добавить ссылки в `SITEMAP` в подходящий раздел (каталог, документы, блог/новости или основные страницы). Для новостей используйте маршруты `/news/<slug>` и `/en/news/<slug>`.

## README-UX

Отдельный файл `README-UX` хранит краткие заметки про анимации, длительность переходов, переключение темы и сценарии работы мобильных фильтров каталога. Этот конспект помогает быстро вспомнить, какие интеракции заложены на сайте и почему.

### Нужны ли дополнительные текстовые документы?
- Базового набора (`README`, `SITEMAP`, `README-UX`) сейчас хватает: он закрывает технику, маршруты и UX-поведение.
- Если понадобится больше регламентов, см. идеи в файле `DOCS-IDEAS.md` (чек-лист публикаций, гид по деплою, стиль контента, доступность и SEO-заметки).
- Добавляйте новые файлы по мере появления процессов, которые нужно закрепить: так документация останется короткой и полезной.

## Полезные команды

| Задача | Команда |
| --- | --- |
| Проверка типов | `pnpm typecheck` |
| Линтер (код-стайл и импорты) | `pnpm lint` |
| Production-сборка | `pnpm build` |
| Проверка production-сборки локально | `pnpm start` |

### Проверка целостности fallback-коллекций

Скрипт `pnpm check:content` проверяет наличие `index.json` и валидность JSON во всех fallback-коллекциях. Запускайте его:

- На CI для каждого Pull Request, чтобы не пропустить битые или неполные записи при ревью.
- Локально перед деплоем или синхронизацией большой пачки контента.
- После массовых правок или импортов (например, миграций через внешний скрипт), чтобы убедиться, что новые данные корректны.

Перед релизом убедитесь, что все проверки (`pnpm typecheck`, `pnpm lint`, `pnpm build`) выполняются без ошибок, а `pnpm start` успешно поднимает собранное приложение.

## Как проверить изменения

Для быстрой проверки локальной разработки:

1. Установите зависимости: `pnpm install`.
2. Запустите типовой набор проверок:
   - Статическая проверка типов: `pnpm typecheck` (удаляет устаревшие типы Next.js перед сборкой).
   - Линтер: `pnpm lint`.
   - Юнит-тесты: `pnpm test` (использует встроенный раннер `tsx --test`).
   - E2E-тесты Playwright: `pnpm pretest:e2e && pnpm test:e2e`.
3. При необходимости соберите проект: `pnpm build`, затем проверьте запуск сборки командой `pnpm start`.

Даже если вы новичок, запуск этих команд по очереди даст уверенность, что страница открывается, формы работают, а сборка не падает.

## Каталог продукции

- Страница списка каталога доступна по адресу `/catalog` (русская локаль) и `/en/catalog` (английская локаль). Фильтры по категории, процессу, основе, наполнителю и металлам собираются из директорий `content/catalog-*` и управляются через Keystatic.
- Детальные страницы продуктов формируются из файлов `content/catalog/<slug>/index.json` и Markdoc-описаний в `content/catalog/<slug>/content/{locale}.mdoc`. Слаг из JSON-файла определяет URL, например: `svyazuyushchee-alpha-pep` → `/catalog/svyazuyushchee-alpha-pep` и `/en/catalog/alpha-pep-binder`.
- Чтобы добавить новый продукт, создайте папку в `content/catalog/`, заполните `index.json` через Keystatic и отредактируйте локализованные `.mdoc` файлы прямо в админке. Keystatic автоматически предложит шаблон и обеспечит предпросмотр страницы.
- Страница `/products` использует категории из `content/catalog-categories/*` и выводит подборки товаров по группам (связующие, покрытия, вспомогательные материалы).
- После публикации нового продукта не забудьте обновить файл `SITEMAP`, чтобы зафиксировать новые маршруты для контента.

## Документы и сертификаты

- Страница «Документы и сертификаты» доступна по адресам `/documents` и `/en/documents`; на ней есть фильтры по типу документа, языку файла и подходящим продуктам.
- Описания страницы (заголовок, подписи фильтров, текст заглушки) лежат в `content/documents-page/index.json` и редактируются через Keystatic.
- Сами карточки документов находятся в `content/documents/<slug>/index.json`. В них можно указать тип (`certificate`, `tds`, `msds`, `brochure`), язык файла (`ru` или `en`), загрузить сам файл и привязать связанные продукты каталога.
- Чтобы добавить документ, откройте коллекцию **Документы** в Keystatic, создайте запись, загрузите файл и, при необходимости, выберите связанные товары. После сохранения документ появится в списке и будет доступен для скачивания.
- При появлении новых документов или изменении маршрутов также вносите ссылки в `SITEMAP` (раздел «Документы и сертификаты»), чтобы поддерживать актуальную карту сайта.

## Сборка и деплой

1. Прогоните `pnpm lint`, `pnpm typecheck` и `pnpm build` на CI или локально.
2. Залейте изменения в основную ветку. Vercel (или другой хостинг) подтянет конфиг и запустит production-сборку.
3. Проверьте, что sitemap (`/sitemap.xml`), Atom-ленты (`/feed.xml`, `/en/feed.xml`) и публичные страницы открываются без ошибок.

## Полезные ссылки

- [Документация Next.js](https://nextjs.org/docs)
- [Документация Keystatic](https://keystatic.com/docs)
